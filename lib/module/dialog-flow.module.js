...
let DialogFlowModule = DialogFlowModule_1 = class DialogFlowModule {
    static forRoute(webHookConfig) {
        webHookConfig = Object.assign({ basePath: 'web-hooks', postPath: 'dialog-glow' }, webHookConfig);
        return {
            module: DialogFlowModule_1,
            controllers: [dialog_flow_controller_1.DialogController.forRoute(webHookConfig)]
        };
    }
    configure(consumer) {
        return consumer
            .apply([dialog_flow_authorization_middleware_1.DialogFlowAuthorizationMiddleware])
            .forRoutes(dialog_flow_controller_1.DialogController);
    }
};
